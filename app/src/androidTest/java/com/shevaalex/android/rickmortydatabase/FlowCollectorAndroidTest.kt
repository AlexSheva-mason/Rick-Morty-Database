 ...
 */
class FlowCollectorAndroidTest<T> {
...
fun <T> Flow<T>.test(scope: CoroutineScope): FlowCollectorAndroidTest<T> {
    val testCollector = FlowCollectorAndroidTest<T>()
    testCollector.testOn(scope, this)
    return testCollector
}

fun <T> FlowCollectorAndroidTest<T>.lastValue(): T? = values.lastOrNull()

fun <T> FlowCollectorAndroidTest<T>.assertEmittedCount(
    count: Int,
...
fun <T> FlowCollectorAndroidTest<T>.assertNothingEmitted(message: String = "Expected nothing emitted") {
    assertEmittedCount(0, message)
}

fun <T> FlowCollectorAndroidTest<T>.assertEmittedValuesEquals(
    values: List<T>,
...
fun <T> FlowCollectorAndroidTest<T>.assertEmittedValuesSame(
    values: List<T>,
    message: String = "Emitted values expectation is not met"
) {
    values
...
fun <T> FlowCollectorAndroidTest<T>.assertEmittedValuesContains(
    value: T,
...
fun <T> FlowCollectorAndroidTest<T>.assertCompleted(message: String = "Expected completed but was not") {
    assertTrue(message, completed)
}

fun <T> FlowCollectorAndroidTest<T>.assertNotCompleted(message: String = "Expected not completed but was") {
    assertFalse(message, completed)
}

fun <T> FlowCollectorAndroidTest<T>.assertNoError(message: String = "Expected no error but there was one") {
    assertNull(message, error)
}

fun <T, E : Throwable> FlowCollectorAndroidTest<T>.assertErrorThrown(
    clazz: KClass<E>,
...
fun <T> FlowCollectorAndroidTest<T>.assertLastEmittedValueEquals(
    v: T,
